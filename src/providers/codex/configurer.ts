  ...
  for (const [matcher, hookPaths] of byMatcher) {
    entries.push({
      ...
      hooks: hookPaths.map((p) => ({
        ...
        command: `bun run ${wrapperPath} ${codexEvent} ${p}`,
      })),
    });
  }
  ...
  if (withoutMatchers.length > 0) {
    const entry: CodexHookEntry = {
      hooks: withoutMatchers.map((h) => ({
        type: "command" as const,
        command: `bun run ${wrapperPath} ${codexEvent} ${h.path}`,
      })),
    };
 ...
 *
 * Usage: bun run shaka-hook-wrapper.ts <eventName> <hookPath>
 *
...
if (!eventName || !hookPath) {
  console.error("Usage: bun run shaka-hook-wrapper.ts <eventName> <hookPath>");
  process.exit(1);
}
...
${markerDeletionBlock}
// Spawn child hook
const child = Bun.spawn(["bun", "run", hookPath], {
...
const cmd = \`sleep 30 && exec bun run "\${workerPath}" "\${markerPath}" "\${timestamp}"\`;
...
for (const hookPath of sessionEndHookPaths) {
  ...
  const proc = Bun.spawn(["bun", "run", hookPath, "--worker", hookTmpPath], {
    stdin: "ignore",
    stdout: "ignore",
