...
describe("ClaudeProviderConfigurer", () => {
  let testClaudeHome: string;
  let testShakaHome: string;
  let testRoot: string | undefined;

  beforeEach(async () => {
    testRoot = await mkdtemp(join(tmpdir(), "shaka-claude-configurer-"));
    testClaudeHome = join(testRoot, "claude");
...
`,
    ...
    // Create critical agent for inference under Shaka's managed agent namespace.
    await Bun.write(
...
`,
  ...
  describe("commands", () => {
    ...
    test("does not treat skipped pre-existing skills as Shaka-managed", async () => {
      ...
      expect(await Bun.file(`${testClaudeHome}/skills/commit/SKILL.md`).text()).toBe(
        "pre-existing",
      );
    });

    test("does not treat user skills that mention the marker as Shaka-managed", async () => {
      const userContent =
        "# User skill\n\nThis example mentions <!-- Generated by Shaka - do not edit manually -->.";
      await mkdir(`${testClaudeHome}/skills/commit`, { recursive: true });
      await Bun.write(`${testClaudeHome}/skills/commit/SKILL.md`, userContent);
      ...
      await installWithCommands(configurer);
      expect(await Bun.file(`${testClaudeHome}/skills/commit/SKILL.md`).text()).toBe(userContent);
    ...
    test("checkInstallation reports missing scoped command files", async () => {
      const projectDir = join(testClaudeHome, "project");
      const commandPath = join(projectDir, ".claude", "skills", "deploy", "SKILL.md");
      await mkdir(projectDir, { recursive: true });
      await mkdir(`${testShakaHome}/system/commands`, { recursive: true });
      await Bun.write(
        `${testShakaHome}/system/commands/deploy.md`,
