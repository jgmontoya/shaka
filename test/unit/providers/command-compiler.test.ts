...
import { compileClaudeCommand } from "../../../src/providers/claude/commands";
import { compileCodexCommand } from "../../../src/providers/codex/commands";
import type { DiscoveredCommand } from "../../../src/providers/command-discovery";
import { compileOpencodeCommand } from "../../../src/providers/opencode/commands";
import { compilePiCommand } from "../../../src/providers/pi/commands";
...
describe("compileForClaude", () => {
  test("produces correct SKILL.md path", () => {
    ...
    const result = compileClaudeCommand(makeCommand(), targetDir);
    expect(result.path).toBe(join(targetDir, "commit", "SKILL.md"));
  });

  test("maps description to frontmatter", () => {
    const result = compileClaudeCommand(makeCommand(), "/home/test/.claude/skills");
  ...
  test("maps argument-hint", () => {
    ...
    const result = compileClaudeCommand(cmd, "/home/test/.claude/skills");
...
describe("compileForOpencode", () => {
  test("produces correct flat file path", () => {
    ...
    const result = compileOpencodeCommand(makeCommand(), targetDir);
    expect(result.path).toBe(join(targetDir, "commit.md"));
  });

  test("maps description to frontmatter", () => {
    const result = compileOpencodeCommand(makeCommand(), "/home/test/.config/opencode/commands");
    expect(result.content).toContain("description: Create a commit");
  });

  test("maps model", () => {
    const cmd = makeCommand({ model: "anthropic/claude-sonnet-4-5" });
    const result = compileOpencodeCommand(cmd, "/home/test/.config/opencode/commands");
...
describe("compileForCodex", () => {
  test("produces SKILL.md path inside named directory", () => {
    ...
    const result = compileCodexCommand(makeCommand(), targetDir);
    expect(result.path).toBe(join(targetDir, "commit", "SKILL.md"));
  });

  test("maps description and argument-hint into description", () => {
    const cmd = makeCommand({ argumentHint: "<message>" });
    const result = compileCodexCommand(cmd, "/home/test/.agents/skills");
...
describe("provider overrides", () => {
  ...
  test("no overrides uses base fields", () => {
    ...
    const claudeResult = compileClaudeCommand(cmd, "/home/test/.claude/skills");
    const opcodeResult = compileOpencodeCommand(cmd, "/home/test/.config/opencode/commands");
...
describe("compileForPi", () => {
  ...
  test("writes to <targetDir>/shaka-<name>.md", () => {
    ...
    const result = compilePiCommand(makeCommand(), targetDir);
  ...
  test("maps argument-hint to frontmatter (Pi supports it natively per Exp 46)", () => {
    const cmd = makeCommand({ argumentHint: "<message>" });
    const result = compilePiCommand(cmd, targetDir);
    expect(result.content).toContain("argument-hint: <message>");
  });
  ...
  test("applies providers.pi override (description + model) over base fields", () => {
    // compilePiCommand runs through `applyOverrides(command, "pi")` so a Pi
