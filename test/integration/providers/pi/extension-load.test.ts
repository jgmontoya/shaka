/**
 * Integration test for the Pi extension generated by `PiProviderConfigurer`.
 *
...
const PI_HOME = join(ROOT, "pi", "agent");
const GENERATED_EXTENSION = join(PI_HOME, "extensions", "shaka.ts");
let importCounter = 0;
...
async function importExtension(): Promise<PiExtensionModule> {
  ...
  const url = `${pathToFileURL(GENERATED_EXTENSION).href}?t=${cacheBust}`;
  return (await import(url)) as PiExtensionModule;
}
...
beforeAll(async () => {
  ...
  await mkdir(join(SHAKA_HOME, "system", "tools"), { recursive: true });
  await mkdir(join(SHAKA_HOME, "system", "agents"), { recursive: true });
  await mkdir(join(SHAKA_HOME, "system", "skills"), { recursive: true });
  await mkdir(join(SHAKA_HOME, "skills"), { recursive: true });
  await Bun.write(
    join(SHAKA_HOME, "system", "tools", "memory-search.ts"),
    `export default {
      name: "memory-search",
  ...
  await Bun.write(
    join(SHAKA_HOME, "system", "tools", "inference.ts"),
    `export default {
      name: "inference",
...
describe.skipIf(process.platform === "win32")("Pi extension — generated extension contract", () => {
  test("uses the Shaka home embedded by the configurer", async () => {
    const mod = await importExtension();
    ...
    expect(memorySearch?.parameters).toEqual(
      expect.objectContaining({
        properties: expect.objectContaining({
          all_projects: expect.objectContaining({ type: "boolean" }),
          type: expect.objectContaining({ enum: ["session", "learning", "knowledge"] }),
        }),
      }),
    );
    const inference = tools.find((tool) => tool.name === "inference");
    expect(inference?.parameters).toEqual(
      expect.objectContaining({
        properties: expect.objectContaining({
          expectJson: expect.objectContaining({ type: "boolean" }),
        }),
      }),
    );
  });
  ...
  test("registered tool execute() keeps the install-time SHAKA_HOME", async () => {
