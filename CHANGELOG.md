...
- **Autoresearch cleanup** — `shaka optimize cleanup` / `shaka autoresearch cleanup` removes experiment worktrees safely after a run. The command is interactive by default, supports worktree-only cleanup with `--worktree --keep-branch`, can remove both worktree and branch with `--worktree --branch`, and refuses dirty worktrees or unmerged branch deletion unless `--force` is explicit
