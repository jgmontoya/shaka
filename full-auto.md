...
4. **Extract `assertOnlySetupDirty(worktreePath)`** — a single helper shared by `commitFinalizeIfDirty` (existing) and `validateSetup` (new). The `SETUP_ARTIFACTS` constant in `src/services/autoresearch.ts` stays the single source of truth; callers must import it, never inline. Prevents drift between validation's dirty-gate and the command layer's.
